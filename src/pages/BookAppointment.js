...
const BookAppointment = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!date || !time || !attendant || !service) {
      Swal.fire({
        icon: "error",
        title: "Oops...",
    ...
    Swal.fire({
      ...
      text: `Do you want to confirm your appointment for ${selectedService.name} with ${selectedAttendant.name} on ${date.toDateString()} at ${time}?`,
      ...
      if (result.isConfirmed) {
        Swal.fire({
          ...
          text: `Your appointment for ${selectedService.name} with ${selectedAttendant.name} has been successfully booked.`,
        });
      }
    });
  };
  ...
  return (
    <Box sx={{ backgroundImage: 'linear-gradient(to right, #f5f7fa, #c3cfe2)', minHeight: '100vh', py: 4 }}>
      <Container maxWidth="lg">
        <Paper elevation={3} sx={{ p: 4, borderRadius: 3, background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)' }}>
          <Typography variant="h4" gutterBottom align="center" sx={{ fontWeight: 'bold', color: '#3a1c71', mb: 4 }}>
            Book Your Salon Experience
          </Typography>

          <Divider sx={{ mb: 4 }}>
            <Chip label={`Step ${step} of 3`} color="primary" sx={{ fontWeight: 'bold' }} />
          </Divider>

          <form onSubmit={handleSubmit}>
            {/* Steps for Date, Time, Service, Attendant as in original code */}
            ...
            {/* When on Step 3, show the Summary and the Confirm button */}
            {step === 3 && (
              <Grid container spacing={4}>
                <Grid item xs={12} md={6}>
                  <Box sx={{ mb: 3 }}>
                        ...
                        <Grid item xs={12} key={att.id}>
                          ...
                          >
                            <Grid container spacing={2} alignItems="center">
                              <Grid item>
                                <Box component="img" src={att.image} alt={att.name} sx={{ width: 50, height: 50, borderRadius: '50%' }} />
                              </Grid>
                              <Grid item xs>
                ...
                <Grid item xs={12} md={6}>
                  <Paper elevation={3} sx={{ p: 3, borderRadius: 2, height: '100%' }}>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Typography variant="body1" fontWeight="medium">{date.toDateString()}</Typography>
                    </Box>
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="subtitle2" color="text.secondary">Time</Typography>
                      <Typography variant="body1" fontWeight="medium">{time || 'Not selected'}</Typography>
                    </Box>
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="subtitle2" color="text.secondary">Service</Typography>
                      <Typography variant="body1" fontWeight="medium">{service ? services.find(s => s.id === service).name : 'Not selected'}</Typography>
                    </Box>
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="subtitle2" color="text.secondary">Attendant</Typography>
                      <Typography variant="body1" fontWeight="medium">{attendant ? attendants.find(a => a.id === attendant).name : 'Not selected'}</Typography>
                    </Box>
                  </Paper>
                </Grid>

                <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                  <Button variant="outlined" color="primary" onClick={handlePreviousStep} sx={{ px: 4, py: 1, borderRadius: 2 }}>
                    Back
                  </Button>
                  <Button type="submit" variant="contained" color="primary" sx={{ px: 4, py: 1, borderRadius: 2 }}>
                    Book Appointment
                  </Button>
                </Grid>
              </Grid>
